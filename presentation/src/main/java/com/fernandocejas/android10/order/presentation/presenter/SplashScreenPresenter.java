...
import com.fernandocejas.android10.order.domain.interactor.buyer.RefreshAccessToken_Buyer;
...
public class SplashScreenPresenter implements Presenter {
    ...
    private final RefreshAccessToken_Buyer refreshAccessTokenUseCase;
    private final PostFirebaseToken postFirebaseTokenUseCase;

    private final GetCountryList getCountryListUseCase;

    @Inject
    public SplashScreenPresenter(RefreshAccessToken_Buyer refreshAccessToken,
...
//        this.splashScreenView.showError(errorMessage);
    ...
    private void navigateToOrderList() {
        if (splashScreenView.activity() instanceof SplashScreenActivity) {
            ((SplashScreenActivity) splashScreenView.activity()).navigateToHome_Buyer();
        }
    }

    private void getUserInformation(String token) {
        refreshAccessTokenUseCase.execute(new RefreshAccessTokenObserver(),
                RefreshAccessToken_Buyer.Params.forRetrieveUserInformation(token));
    }
    ...
    private final class RefreshAccessTokenObserver extends DefaultObserver<User_Buyer> {
