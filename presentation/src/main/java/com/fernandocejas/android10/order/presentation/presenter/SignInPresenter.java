...
import com.fernandocejas.android10.order.domain.interactor.buyer.SignIn_Buyer;
...
public class SignInPresenter implements Presenter {
    ...
    @Inject
    public SignInPresenter(SignIn_Buyer signInUseCase,
    ...
    public void navigateToOrderList() {
        if (signInView.activity() instanceof SignInActivity) {
            ((SignInActivity) signInView.activity()).navigateToHome_Buyer();
        }
    }

    public void navigateToCompany() {
        if (signInView.activity() instanceof SignInActivity) {
            ((SignInActivity) signInView.activity()).navigateToCompany_Buyer();
        }
    }
...
//                    SignInPresenter.this.sendRegistrationToServer(user.getToken());
                    if (user.document.equals("0")) {// if document is '0' change to Company Screen
                        SignInPresenter.this.navigateToCompany();
                    } else {
                        if (!is_recall) {
                            //go to order list
