...
import com.fernandocejas.android10.order.domain.interactor.buyer.ChangePassword_Buyer;
import com.fernandocejas.android10.order.domain.interactor.buyer.RetrieveUserInformation_Buyer;
import com.fernandocejas.android10.order.domain.interactor.buyer.UpdateUserInfo_Buyer;
...
public class AccountInformationPresenter implements Presenter {
    ...
    private final RetrieveUserInformation_Buyer retrieveUserInformationUseCase;
    private final ChangePassword_Buyer changePasswordUseCase;
    private final UpdateUserInfo_Buyer updateUserInfoUseCase;
    private final UserModelDataMapper_Buyer userModelDataMapper;
    private User_Buyer user;

    @Inject
    public AccountInformationPresenter(
            UserModelDataMapper_Buyer userModelDataMapper,
            RetrieveUserInformation_Buyer retrieveUserInformationUseCase,
            ChangePassword_Buyer changePasswordUseCase,
            UpdateUserInfo_Buyer updateUserInfoUseCase) {
    ...
    private void setUser(User_Buyer user) {
        if (user == null) {
            return;
        }
        this.user = user;
    ...
    private void changePassword(String old_password, String password) {
        ...
        this.changePasswordUseCase.execute(new ChangePasswordObserver(), ChangePassword_Buyer.Params.forChangePassword(access_token, old_password, password));
    }
    ...
    private void updateUserInfo(String name, String phone) {
        ...
        //
        this.updateUserInfoUseCase.execute(new UpdateUserInfoObserver(), UpdateUserInfo_Buyer.Params.forUpdateUserInfo(access_token, name, phone, user.getAvatar()));
    }
    ...
    private void getUserInfo() {
        String access_token = PreferencesUtility.getInstance(this.accountInformationView.context())
                .readString(PreferencesUtility.PREF_TOKEN, null);
        this.retrieveUserInformationUseCase.execute(new RetrieveAccessTokenObserver(), RetrieveUserInformation_Buyer.Params.forRetrieveUserInformation(access_token));
    }
    ...
    private final class RetrieveAccessTokenObserver extends DefaultObserver<User_Buyer> {
    ...
    private final class ChangePasswordObserver extends DefaultObserver<User_Buyer> {
    ...
    private final class UpdateUserInfoObserver extends DefaultObserver<User_Buyer> {
