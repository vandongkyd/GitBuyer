...
import com.fernandocejas.android10.order.domain.interactor.buyer.ActiveUser_Buyer;
import com.fernandocejas.android10.order.domain.interactor.buyer.ResendActiveCode_Buyer;
...
public class ActivationPresenter implements Presenter {
    ...
    private final ResendActiveCode_Buyer resendActiveCodeUseCase;
    private final PostFirebaseToken postFirebaseTokenUseCase;

    private boolean is_recall;

    @Inject
    public ActivationPresenter(ActiveUser_Buyer activeUserUseCase,
                               ResendActiveCode_Buyer resendActiveCodeUseCase,
    ...
    public void navigateToOrderList() {
        if (activationView.activity() instanceof ActivationActivity) {
            ((ActivationActivity) activationView.activity()).navigateToHome_Buyer();
        }
    }

    public void navigateToCompany() {
        if (activationView.activity() instanceof ActivationActivity) {
            ((ActivationActivity) activationView.activity()).navigateToCompany_Buyer();
        }
    }
    ...
    private void activateCode(String token, String code) {
        this.activeUserUseCase.execute(new ActivationPresenter.ActivationObserver(), ActiveUser_Buyer.Params.forActiveUser(token, code));
    }

    private void resendActiveCode(String token) {
        this.resendActiveCodeUseCase.execute(new ResendActiveCodeObserver(), ResendActiveCode_Buyer.Params.forResendActiveCode(token));
    }
...
//                    ActivationPresenter.this.sendRegistrationToServer(user.getToken());
                    ...
                    if (user.document.equals("0")) {// if document is '0' change to Company Screen
                        ActivationPresenter.this.navigateToCompany();
                    }else {
                        if (!is_recall) {
                            //go to order list
    ...
    private final class ResendActiveCodeObserver extends DefaultObserver<User_Buyer> {
